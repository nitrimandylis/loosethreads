...
- **Three ways to close a board again.** Replacing the passphrase changes the word; signing everyone out leaves the word alone and invalidates every cookie, so the people inside have to type it again; deleting takes the board and everything pinned to it away for good. All three are buttons in `/admin`, and the delete confirmation names how many notes are about to go, because "delete this board" and "delete these forty rumours" are not the same decision. There is no undo and no record kept, which is the same deal the rest of the board runs on.
